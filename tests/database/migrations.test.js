...
describe('Database Migrations', () => {
  ...
  describe('Edge Cases', () => {
    ...
    test('EC-7: should order blocks by position within page', async () => {
      ...
      expect(JSON.parse(JSON.stringify(result.rows[0].content)).text).toBe('first');
      expect(JSON.parse(JSON.stringify(result.rows[1].content)).text).toBe('second');
      expect(JSON.parse(JSON.stringify(result.rows[2].content)).text).toBe('third');
    });
  });
});
